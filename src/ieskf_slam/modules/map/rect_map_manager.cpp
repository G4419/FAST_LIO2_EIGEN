...
namespace IESKFSlam
{
    ...
    void RectMapManager::addScan(PCLPointCloudPtr cur_scan, IESKF::Ptr ieskf_ptr){
        ...
        if(local_map_ptr->empty()) *local_map_ptr = scan;
            ...
            while(right > left){
                while(right > left && 
                        abs(local_map_ptr->points[left].x - x.position.x()) < map_side_length_2 &&
                        abs(local_map_ptr->points[left].y - x.position.y()) < map_side_length_2 &&
                        abs(local_map_ptr->points[left].z - x.position.z()) < map_side_length_2) left++;
                while(right > left && 
                        abs(local_map_ptr->points[right].x - x.position.x()) > map_side_length_2 &&
                        abs(local_map_ptr->points[right].y - x.position.y()) > map_side_length_2 &&
                        abs(local_map_ptr->points[right].z - x.position.z()) > map_side_length_2) right--;
